...
export default function SearchTripsScreen() {
    ...
    const [activeFilter, setActiveFilter] = useState('Viajes hoy');
    const [origen, setOrigen] = useState('');
    const [destino, setDestino] = useState('');
    const [puntoEncuentro, setPuntoEncuentro] = useState('');
    const handleReserve = (tripId, price, destination) => {
        var _a;
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.searchCard}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput placeholder="Campus Peñalolén" style={styles.input} placeholderTextColor="#94a3b8" value={origen} onChangeText={setOrigen}/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Destino</Text>
            <TextInput placeholder="¿Hacia dónde vamos?" style={styles.input} placeholderTextColor="#94a3b8" value={destino} onChangeText={setDestino}/>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Punto de encuentro</Text>
            <TextInput placeholder="Metro Grecia" style={styles.input} placeholderTextColor="#94a3b8" value={puntoEncuentro} onChangeText={setPuntoEncuentro}/>
          </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tagsRow}>
          {FILTER_TAGS.map((tag) => (<TouchableOpacity key={tag} style={[styles.tagChip, activeFilter === tag && styles.tagChipActive]} onPress={() => setActiveFilter(tag)}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.campusGrid}>
            {CAMPUS_LOCATIONS.map((campus) => (<TouchableOpacity key={campus} style={[styles.campusCard, campus === selectedCampus && styles.campusCardActive]} onPress={() => setSelectedCampus(campus)}>
        ...
        <View style={styles.section}>
          ...
          {filteredTrips.map((trip) => (<View key={trip.id} style={styles.tripCard}>
              ...
              <View style={styles.tripActions}>
                <TouchableOpacity style={styles.secondaryButton} onPress={() => router.push({ pathname: '/trip/[id]', params: { id: trip.id } })}>
                  <Text style={styles.secondaryButtonText}>Detalle</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.primaryButtonSmall} onPress={() => handleReserve(trip.id, trip.precioCLP, trip.destinoCampus)}>
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            var _a;
            return (<View key={driver.id} style={styles.driverCard}>
                <Text style={styles.driverName}>{driver.name}</Text>
                <Text style={styles.driverCar}>{driver.car}</Text>
                <View style={styles.driverMetaRow}>
                  <Text style={styles.driverMeta}>{driver.completedTrips} viajes</Text>
                  <View style={styles.metaDot}/>
                  <Text style={styles.driverMeta}>{(_a = driver.rating) === null || _a === void 0 ? void 0 : _a.toFixed(1)} ★</Text>
                </View>
                <View style={styles.driverTagRow}>
                  {driver.expertise.map((tag) => (<View key={tag} style={styles.driverTag}>
