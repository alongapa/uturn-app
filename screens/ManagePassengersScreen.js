...
function statusBackground(state) {
    if (state === 'confirmado')
...
export default function ManagePassengersScreen() {
    const [selectedPassenger, setSelectedPassenger] = useState(PASSENGER_MANIFEST.length > 0 ? PASSENGER_MANIFEST[0].id : null);
    return (<SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {PASSENGER_MANIFEST.map((passenger) => {
            var _a;
            return (<TouchableOpacity key={passenger.id} style={[styles.card, selectedPassenger === passenger.id && styles.cardActive]} onPress={() => setSelectedPassenger(passenger.id)} activeOpacity={0.9}>
            ...
            {passenger.badges && (<View style={styles.badgeRow}>
                {passenger.badges.map((badge, i) => (<View key={badge + i} style={styles.badge}>
            ...
            <View style={styles.insightRow}>
              ...
              <View>
                ...
                <Text style={styles.insightValue}>
                  {((_a = passenger.badges) === null || _a === void 0 ? void 0 : _a.includes('Pago verificado')) ? 'Verificado' : 'Pendiente'}
                </Text>
              </View>
            </View>
