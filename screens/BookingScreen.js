...
const getBookingStatusBackground = (state) => {
    if (state === 'confirmado') {
...
export default function BookingScreen() {
    ...
    const [selectedUniversity, setSelectedUniversity] = useState((_b = (_a = UNIVERSITIES[0]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : 'uai');
    const [originCampusId, setOriginCampusId] = useState((_d = (_c = CAMPUSES[0]) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : '');
    const [destinationCampusId, setDestinationCampusId] = useState((_f = (_e = CAMPUSES[1]) === null || _e === void 0 ? void 0 : _e.id) !== null && _f !== void 0 ? _f : '');
    ...
    useEffect(() => {
        ...
        if (nextOrigin) {
            ...
            setMeetingPointId((_b = (_a = nextOrigin.meetingPoints[0]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : '');
        }
    }, [selectedUniversity]);
    useEffect(() => {
        var _a, _b;
        ...
        if (originCampus && !originCampus.meetingPoints.find((point) => point.id === meetingPointId)) {
            setMeetingPointId((_b = (_a = originCampus.meetingPoints[0]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : '');
        }
    ...
    const originCampus = CAMPUSES.find((campus) => campus.id === originCampusId);
    const meetingPointOptions = (_g = originCampus === null || originCampus === void 0 ? void 0 : originCampus.meetingPoints) !== null && _g !== void 0 ? _g : [];
    const handleCreateTrip = () => {
        ...
        const hasVehicleInfo = Boolean(((_a = user.vehicle) === null || _a === void 0 ? void 0 : _a.brand) && ((_b = user.vehicle) === null || _b === void 0 ? void 0 : _b.model) && ((_c = user.vehicle) === null || _c === void 0 ? void 0 : _c.year));
        const hasLicense = Boolean(user.driverLicenseNumber);
        if (!hasVehicleInfo || !hasLicense) {
            Alert.alert('Completa tu perfil de conductor', 'Necesitas registrar tu vehículo y licencia antes de publicar viajes.', [
                {
                    text: 'Ir al perfil',
        ...
        const newTrip = {
            ...
            meetPoint: (_d = meetingPoint === null || meetingPoint === void 0 ? void 0 : meetingPoint.name) !== null && _d !== void 0 ? _d : 'Punto coordinado',
            price: Number(tripPrice) || 0,
            seats: Number(tripSeats) || 1,
            departAt: departAt || new Date().toISOString(),
            routeNotes: `Coordinado desde ${(_e = originCampus === null || originCampus === void 0 ? void 0 : originCampus.name) !== null && _e !== void 0 ? _e : 'campus'}`,
            originCampusId,
            destinationCampusId,
            meetingPointId,
        };
    ...
    return (<SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.formCard}>
          ...
          <SelectField label="Universidad" value={selectedUniversity} onSelect={setSelectedUniversity} options={UNIVERSITIES.map((item) => ({ label: item.name, value: item.id }))} placeholder="Selecciona universidad"/>
          <SelectField label="Campus de origen" value={originCampusId} onSelect={setOriginCampusId} options={CAMPUSES.filter((campus) => campus.universityId === selectedUniversity).map((campus) => ({
            label: campus.name,
            value: campus.id,
        }))} placeholder="Selecciona campus"/>
          <SelectField label="Campus destino" value={destinationCampusId} onSelect={setDestinationCampusId} options={CAMPUSES.map((campus) => ({ label: campus.name, value: campus.id }))} placeholder="Selecciona campus"/>
          <SelectField label="Punto de encuentro" value={meetingPointId} onSelect={setMeetingPointId} options={meetingPointOptions.map((point) => ({ label: point.name, value: point.id }))} placeholder="Selecciona punto"/>
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Destino final</Text>
            <TextInput value={tripDestination} onChangeText={setTripDestination} placeholder="Providencia, Ñuñoa..." style={styles.input} placeholderTextColor="#94a3b8"/>
          </View>
          <View style={styles.rowInputs}>
            <View style={styles.rowItem}>
              <Text style={styles.label}>Tarifa (CLP)</Text>
              <TextInput value={tripPrice} onChangeText={setTripPrice} placeholder="2500" style={styles.input} keyboardType="numeric" placeholderTextColor="#94a3b8"/>
            </View>
            <View style={styles.rowItem}>
              <Text style={styles.label}>Asientos</Text>
              <TextInput value={tripSeats} onChangeText={setTripSeats} placeholder="3" style={styles.input} keyboardType="numeric" placeholderTextColor="#94a3b8"/>
            </View>
          </View>
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Salida (ISO)</Text>
            <TextInput value={departAt} onChangeText={setDepartAt} placeholder="2024-10-31T07:45:00-03:00" style={styles.input} placeholderTextColor="#94a3b8"/>
          </View>
        ...
        {createdTrips.length > 0 && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>Viajes publicados</Text>
            {createdTrips.map((trip) => (<View key={trip.id} style={styles.card}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
          {STATUS_FILTERS.map((filter) => (<TouchableOpacity key={filter} onPress={() => setSelectedFilter(filter)} style={[styles.filterChip, selectedFilter === filter && styles.filterChipActive]}>
        ...
        {filteredBookings.map((booking) => (<View key={booking.id} style={styles.card}>
            <View style={styles.cardHeader}>
...
function SelectField({ label, value, placeholder, options, onSelect }) {
    ...
    const selectedLabel = (_a = options.find((option) => option.value === value)) === null || _a === void 0 ? void 0 : _a.label;
    return (<View style={styles.inputGroup}>
      ...
      {open && (<View style={styles.dropdown}>
          {options.map((option) => (<TouchableOpacity key={option.value} style={styles.dropdownOption} onPress={() => {
